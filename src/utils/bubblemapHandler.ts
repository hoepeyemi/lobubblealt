...
// Using a global Map to store bubblemap requests (will be reset on serverless function cold starts)
const userBubblemapRequests = new Map<number, BubblemapRequest>();

// Persistent storage functions for bubblemap requests in serverless environments
// This could be expanded to use a database or other persistent storage methods
export function storeBubblemapRequest(chatId: number, request: BubblemapRequest): void {
  userBubblemapRequests.set(chatId, request);
}

export function getBubblemapRequest(chatId: number): BubblemapRequest | undefined {
  return userBubblemapRequests.get(chatId);
}

export function deleteBubblemapRequest(chatId: number): void {
...
export async function handleBubblemapCommand(bot: TelegramBot, msg: TelegramBot.Message): Promise<void> {
  ...
  if (parts.length > 1) {
    ...
    storeBubblemapRequest(chatId, { stage: 'WAITING_FOR_TOKEN' });
...
export async function handleBubblemapConversation(bot: TelegramBot, msg: TelegramBot.Message): Promise<boolean> {
  ...
  const request = getBubblemapRequest(chatId);
